...
import { Plus, LogIn, LogOut, Shield, Users, Map, User, Flame, Trophy } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { streak, updateStreak } = useGameData();
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      await supabase.from("room_members").insert({
        room_id: room.id, user_id: user!.id, color: USER_COLORS[0],
      });
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (error || !joinedRoomId) throw new Error(error?.message || "Room not found");
  ...
  return (
    <div className="min-h-screen bg-background bg-grid-pattern">
          ...
          <div className="flex items-center gap-3">
            {streak && (
              <div className="hidden md:flex items-center gap-2 text-sm">
                ...
                <span className="font-display font-bold text-destructive">{streak.current_streak}</span>
              </div>
            )}
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {showCreate && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="bg-card border border-border rounded-xl p-6 mb-8">
            <h3 className="font-display text-lg mb-4">Create New Room</h3>
            <div className="flex gap-3">
              <Input value={roomName} onChange={(e) => setRoomName(e.target.value)} placeholder="Room name (e.g. Biology 101)" className="bg-secondary border-border flex-1" />
        ...
        {showJoin && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="bg-card border border-border rounded-xl p-6 mb-8">
            ...
            <div className="flex gap-3">
              <Input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())} placeholder="Enter 6-character code" maxLength={6} className="bg-secondary border-border flex-1 font-mono uppercase tracking-widest" />
          ...
          {rooms.map((room, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: room.myColor }} />
              </div>
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span className="font-mono text-xs bg-secondary px-2 py-0.5 rounded">{room.room_code}</span>
